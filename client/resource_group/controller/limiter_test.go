...
func TestRefundTokensReflowsFutureReservations(t *testing.T) {
	re := require.New(t)
	resetTime()
	lim := NewLimiter(t0, 100, 0, 0, make(chan notifyMsg, 1))
	ctx := context.Background()
	...
	lim.RefundTokens(t1, 5000)

	for i := range oldReservations {
		re.Equal(0*time.Second, oldReservations[i].DelayFrom(t1),
			"existing sleeping reservations should be reflowed after RefundTokens")
	}
...
func TestRefundTokensWakesFutureReservationWaiter(t *testing.T) {
	...
	time.Sleep(20 * time.Millisecond)
	lim.RefundTokens(now.Add(20*time.Millisecond), 1000)

	select {
	case err := <-resultCh:
		re.NoError(err)
	case <-time.After(300 * time.Millisecond):
		re.Fail("refund should reflow and wake the existing future reservation")
	}
}
